...
## Prenosná SQL schéma (database_schema.sql)

Súbor `database_schema.sql` obsahuje prenosnú (vendor‑neutral) definíciu schémy, ktorú je možné použiť aj mimo PostgreSQL. Primárne kľúče sú typu `INTEGER` bez špecifickej auto‑increment syntaxe, aby bol skript kompatibilný s viacerými SŘBD. Podľa zvoleného systému doplňte auto‑increment/identity pre stĺpce `Id` takto:

- PostgreSQL: `Id INTEGER GENERATED BY DEFAULT AS IDENTITY`
- SQL Server: `Id INT IDENTITY(1,1)`
- MySQL/MariaDB: `Id INT AUTO_INCREMENT`
- SQLite: `Id INTEGER PRIMARY KEY AUTOINCREMENT`

Ručné spustenie skriptu (príklady):

```bash
# PostgreSQL
psql -h localhost -U postgres -d kros_assignment_db -f database_schema.sql

# SQL Server (sqlcmd)
sqlcmd -S localhost -d KrosAssignment -i database_schema.sql

# MySQL/MariaDB
mysql -h localhost -u root -p kros_assignment_db < database_schema.sql

# SQLite
sqlite3 kros_assignment.db < database_schema.sql
```

Poznámka: V produkcii odporúčame používať EF Core migrácie. SQL skript je vhodný na rýchlu inicializáciu v rôznych databázach.
...
### Povinné polia
- **Company**: Code, Name, LeaderId
- **Division**: Code, Name, CompanyId, LeaderId
- **Project**: Code, Name, DivisionId, LeaderId
- **Department**: Code, Name, ProjectId, LeaderId
...
- Každá Company/Division/Project/Department musí mať vždy nastaveného vedúceho (LeaderId)
